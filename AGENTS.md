...
## Development Discipline (nodumb)

Код — дешёвая часть ошибки. Дороже всего уверенно делать не то, не там или на
основании факта, которого никто не проверял. Эти правила применяются ко всем
нетривиальным задачам в этом репо.
...
- Сформулировать задачу от пользователя, а не от интерфейса или кода. Запрос
  часто приходит уже в форме решения — проверить, ту ли проблему оно решает.
- Проверить масштаб: локальная правка может скрывать общее решение, а общий слой
  может оказаться лишним.
- Назвать признак успеха, который можно наблюдать (тест, поведение, лог).
- Найти самую дорогую развилку и дать по ней одну рекомендацию.
- Явно согласовывать только необратимое решение, существенную неопределённость
  или изменение скоупа. В остальных случаях назвать допущение и продолжить.
- Не задавать вопрос, если можно безопасно сделать допущение. Назвать его и
  продолжить.
- Получать явное подтверждение перед внешним, публичным, финансовым или трудно
  отменяемым действием (push, release, отправка в шлюз/платформу).

### Перед массовой правкой

Применять, если одно изменение приходится повторять в нескольких независимых
местах (рефакторинг, миграция, унификация, переименование).

- Сначала посчитать места и значения, потом оценивать объём.
- Проверить, действительно ли повторения имеют одну причину меняться. Если
  причины разные — оставить решения локальными.
- Если причина общая — предложить минимальный источник (контракт, токен, хелпер,
  адаптер, скрипт). Не заводить слой «на будущее»: нормальный результат проверки
  — **общий слой пока не нужен**.
- Начать с одного небольшого участка и проверить его до массового переноса.

### Когда отладка встала

Перед второй правкой получить новый факт. После двух неудачных правок считать,
что прежний метод исчерпан.

- Сначала воспроизвести симптом на той ветке/сессии, где он возникает.
...
- Сменить источник данных: лог, дамп состояния, минимальный пример, история
  изменений, `tests/`, issue-трекер или рабочий аналог. Только после нового
  наблюдения менять код.
- Если «починка» была объявлена, но не подтвердилась — сначала заменить способ
  проверки, а не делать ещё одну правку. Ложная победа — сигнал менять метод.

### Как отвечать

- Начинать с прямого ответа, вывода или рекомендуемого действия. По умолчанию
  укладываться в 4–5 насыщенных предложений; не добавлять заголовки и списки,
  если мысль помещается в абзац.
- Коротко назвать: что известно точно; какое предположение сейчас самое дорогое;
  что делать следующим шагом; какой результат подтвердит или опровергнет решение.
  Это не обязательный шаблон — выбирать то, что важнее для текущего режима.
- Отделять факт, вывод и неопределённость, когда их легко перепутать. Для важных,
  спорных или меняющихся фактов проверять первичный источник и ставить ссылку.
...
- Не использовать пустую похвалу, канцелярит и повторение запроса. Подстраивать
  язык и тон под пользователя.

## Changelog Discipline

CHANGELOG — не список изменений, а **журнал решений**. В этом репо журнал решений
ведётся в локальном `docs/plans/` (каталог gitignored — рабочие журналы не
коммитятся) и в содержательных сообщениях коммитов.

- **Пишется для того, кто вернётся через полгода.** Обычно это сам автор,
  забывший контекст.
- **«Почему» дороже «что».** Что изменилось — видно в диффе. Почему выбрали этот
  вариант — не видно нигде.
- **Отвергнутая альтернатива ценнее описания принятой.** Она не даст через полгода
  переделать обратно и наступить на те же грабли.
- **Запись делается сразу**, а не перед релизом. Через неделю причина решения
  забыта, останется пересказ диффа.
- При создании плана в `docs/plans/` указывать: что меняем, где, почему именно так,
  как было раньше, что рассмотрели и отвергли. Не ограничиваться пересказом диффа.

## Feedback в интерфейсах (TUI / dashboard / gateway)

hermes-agent — это в первую очередь бэкенд и CLI, но у него есть пользовательские
поверхности: TUI (`ui-tui`), dashboard (`hermes dashboard`), и сообщения в
мессенджеры через gateway. Для них действует правило:

- **Действие без ответа — сломанное действие.** Если пользователь (или внешний
  вызов) что-то сделал и не получил видимого подтверждения исхода — он повторит
  действие и получит дубль или ошибку.
- Ответ должен быть заметен там, куда смотрит человек, а не там, где удобно
  разработчику. Молчание читается как «не сработало».
- Сообщение об ошибке должно говорить **что делать дальше**, а не только что
  случилось. Без внутренних терминов и кодов, если их можно избежать.
- Необратимое действие требует подтверждения с названным последствием («будет
  удалено 12 записей»), а не «вы уверены?». Обратимое — лучше undo, чем confirm.
