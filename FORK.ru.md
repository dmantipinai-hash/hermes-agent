...
- **У форка своя линия релизов: `v1.1` и далее** (GitHub Releases этого
  репозитория). `v1.1` (31.08.2026) — релизный срез с Архитектурой 2.0:
  память v2, мягкая доставка сообщений, оркестрация, осознанность
  (DETECT→RECORD + карта напряжений), экономика токенов. Следующие релизы
  продолжают линию 1.x; обновляться **только из этого репозитория**.
