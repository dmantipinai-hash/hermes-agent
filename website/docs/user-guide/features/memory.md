...
`supersede` adds the new entry, deprecates the old one and writes the provenance link in a single transaction — no half-completed changes, no substring quoting. The `old_id`/`superseded_by_id` parameters accept the full UUID **or an unambiguous 8+ char prefix** — the short id form shown in retrieval results (pack bullets, recall neighbors) works directly; an ambiguous prefix fails with the match count and changes nothing. Ids also resolve **across the two stores**: when the exact id belongs to the other target than the one addressed (a live failure loop: an entry in `user` mutated as `memory` four times), the operation applies to the entry's actual store and the response carries a `target corrected` note. `old_text` must match the right store — but a miss now says which target holds the text (`An entry matching this text exists in target 'user' — retry with target='user'`), and pack lines tag user-store entries with `· user` so the store is visible before the first call.
