...
class GatewayKanbanWatchersMixin:
        ...
        while self._running:
            try:
                ...
                for d in deliveries:
                    ...
                    # A normal named-profile gateway owns its adapters directly
                    # on ``self.adapters`` just like the default gateway. Only a
                    # DIFFERENT multiplexed profile belongs in
                    # ``self._profile_adapters``. Passing this gateway's own
                    # stamped name to _authorization_adapter would therefore
                    # look in the secondary registry and rewind forever.
                    adapter_profile = (
                        None if sub_profile == notifier_profile else sub_profile or None
                    )
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        try:
            ...
            lock_handle = self._kanban_dispatcher_lock_handle
            self._kanban_dispatcher_lock_handle = None
