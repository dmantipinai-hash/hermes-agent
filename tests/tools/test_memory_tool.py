...
class TestMemoryBatch:
    ...
    @pytest.mark.xfail(reason='fork deviation: memory v2 typed store applies operations batches '
                          'sequentially (per-op budget), not atomically with final-budget semantics', strict=False)
    ...
    @pytest.mark.xfail(reason='fork deviation: v2 batches are sequential; a blocked op does not roll back prior ops', strict=False)
