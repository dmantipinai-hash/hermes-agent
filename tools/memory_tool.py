...
def memory_tool(
    ...
    # Upstream-compatible entry normalisation: ``new_text`` is an alias for
    # ``content``; ``operations`` applies a batch sequentially against the
    # same v2 store paths (the upstream atomic final-budget check degrades to
    # per-op budget checks — the typed store enforces its own budget anyway).
    if new_text is not None and content is None:
        content = new_text
    if operations:
        ...
        for op in operations:
            results.append(
                memory_tool(
                    ...
                    target=target,
                    content=op.get("content", op.get("new_text")),
        ...
        failed = [r for r in results if '"success": false' in r or '"success":false' in r]
        if failed:
            return json.dumps({
                "success": False,
                "error": "one or more batch operations failed",
...
MEMORY_SCHEMA = {
    ...
    "description": (
        ...
        "Check the entry's OWN target before mutating it: read results carry a 'target' field per\n"
        "entry, and pack lines tag user-store entries with '· user'. An exact old_id still resolves\n"
        "across stores (the response notes the corrected target), but old_text must match the right\n"
        "store — a miss there tells you which target holds the text.\n\n"
        ...
        "the budget are evicted from the PROMPT only. Eviction is AUTOMATIC and the "
        "store is unbounded: NEVER remove, shorten, or archive-and-delete entries to "
        "'free space' — the overflow simply lives in the cold tier and stays searchable. "
        "The cold tier is the full typed "
