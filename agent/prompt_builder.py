...
KANBAN_GUIDANCE = (
    ...
    "3a. **Check the thread at checkpoints (mailbox).** Before `kanban_block` or "
    "`kanban_complete`, and after long heartbeats, call "
    "`read_task_thread(since=<last seen id>)`; do not poll every iteration. Other "
    "agents/managers mark questions with `[question]`. Answer each first with "
    "`kanban_comment(body=\"[answer] ...\")`; never complete past an unanswered "
    "question. Save each response's `last_comment_id` and use it as the next "
    "`since` value to fetch only new comments.\n"
