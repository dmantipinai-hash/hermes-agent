...
_MEMORY_REVIEW_PROMPT = (
    ...
    "3. Has the user MADE A DECISION (chose X over Y, rejected an approach, approved "
    "a direction) or STATED A CONSTRAINT/RULE ('never do X', 'always Y here')?\n\n"
    "TYPED SAVING — use the type parameter when it fits:\n"
    "- Decisions → memory(action=add, type=decision): include WHAT was decided and "
    ...
    "reason=<why superseded>), then save the new entry. If add returns "
    "'related_active', review those entries for the same conflict.\n\n"
    "If something stands out, save it using the memory tool. "
    "If nothing is worth saving, just say 'Nothing to save.' and stop — "
    "a no-op is a normal outcome."
)
